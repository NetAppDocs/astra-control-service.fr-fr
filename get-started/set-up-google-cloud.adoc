...
Clusters privés:: Si le cluster est privé, le https://["réseaux autorisés"^] Doit autoriser l'adresse IP du service de contrôle Astra :
+
--
52.188.218.166/32
...
.Étape
. Accédez au https://["Page NetApp Cloud Volumes Service"^] Dans Google Cloud Marketplace, sélectionnez *Acheter* et suivez les invites.
+
https://["Suivez des instructions détaillées dans la documentation Google Cloud pour acheter et activer le service"^].
...
.Étape
. https://["Utilisez la console Google Cloud ou l'interface de ligne de commande gCloud pour activer les API suivantes"^]:
+
** Google Kubernetes Engine
** Le stockage cloud
** API JSON de stockage cloud
...
.Étapes
. Rendez-vous sur Google Cloud et https://["créez un compte de service à l'aide de la console, de la commande gcloud ou d'une autre méthode préférée"^].
...
.Étapes
. Rendez-vous sur Google Cloud et https://["créez une clé de compte de service à l'aide de la console, de la commande gcloud ou d'une autre méthode préférée"^].
...
.Étapes
. https://["Accédez à NetApp Cloud Central's Global régions Maps"^] Et identifiez le type de service que vous allez utiliser dans la région Google Cloud où se trouve votre cluster.
+
Cloud Volumes Service propose deux types de services : CVS et CVS-Performance. https://["En savoir plus sur ces types de service"^].

. https://["Accédez à Cloud volumes dans Google Cloud Platform"^].
...
+
Pour plus de détails sur l'utilisation de ces commandes, reportez-vous au https://["Service de démarrage rapide pour Cloud Volumes Service pour GCP"^].

+
https://["En savoir plus sur la configuration de l'accès aux services privés et la configuration du peering de réseau"^].
