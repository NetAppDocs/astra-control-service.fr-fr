...
. Appliquez le `astraconnector_operator.yaml` fichier. Lorsque vous exécutez cette commande, le namespace correct pour l'opérateur Astra Connector est créé et la configuration est appliquée à l'espace de noms :
+
[source, console]
----
kubectl apply -f astraconnector_operator.yaml
----
. Créer un namespace pour les composants du cluster privé :
+
[source, console]
----
...
. Appliquer la définition de ressource personnalisée (CRD) d'Astra Connector :
+
[source, console]
----
