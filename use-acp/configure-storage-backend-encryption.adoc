...
.Avant de commencer
* Assurez-vous d'avoir activé Astra Control Provisioner sur le cluster géré. Reportez-vous à la section link:../use/enable-acp.html["Activez le mécanisme de provisionnement Astra Control"^] pour obtenir des instructions.
...
Une fois que vous avez créé un système back-end et une classe de stockage, vous pouvez provisionner un volume. Pour obtenir des instructions, reportez-vous à la section https://docs.netapp.com/us-en/trident/trident-use/vol-provision.html["Provisionner un volume"^].
